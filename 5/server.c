...
int main(){
	...
	while(1){ //try to accept new client request
		...
		//printf("\t IP = %s   port = %d\n", inet_ntoa(client_addr.sin_addr.s_addr), ntohs(client_addr.sin_port)); //breaks on this but works fine without, maybe client info not set in client( this is with book way so idk)
